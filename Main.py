...
class WindowsAddBattle(QMainWindow, Ui_Frame_add_battle):
    ...
    def re_b(self):
        try:
            ...
            QMessageBox.information(self, "护肝宝-编队-错误", e.message, QMessageBox.Yes)
        except Exception as e:
            QMessageBox.information(self, "护肝宝-编队-错误", str(e), QMessageBox.Yes)
...
class WindowsMain(QMainWindow, Ui_Frame_main):
    ...
    def __init__(self):
        ...
        self.setupUi(self)
        self.setWindowTitle('护肝宝 Beta' + str(VERSION) + " 本软件永久免费, 任意售卖均未经许可")
        self.menu_event()
    ...
    def menu_event(self):
        ...
        action_about_author.triggered.connect(lambda: QMessageBox.information(self, '护肝宝-关于',
                                                                              '版本:Beta-' + str(VERSION)
                                                                              + '\n作者: Simon菌'
                                                                              + '\nQQ群:560271993'
...
class OtherFunction:
    ...
    def show_mine(windows):
        try:
            QMessageBox.information(windows, '护肝宝-我的',
    ...
    def get_log(windows):
        log.get_log()
        QMessageBox.information(windows, "护肝宝", "已将日志输出至桌面!", QMessageBox.Yes)
    ...
    def dismantle_equipment(item):
        if item == 0:
            rep = QMessageBox.question(windows_main, '护肝宝', '将会自动分解三星以下的装备\n请提前上锁重要装备\n如:绿声呐,绿投弹\n是否继续?', QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        ...
        if 'version' in gameData.mine:
            if gameData.mine['version'] > VERSION:  # 版本过期
                ...
                if data != '':
                    speak += '\n更新日志:' + data
                speak += '\n是否去下载最新版本?'
                reply = QMessageBox.question(windows_main, '护肝宝', speak, QMessageBox.Yes | QMessageBox.No,
                                             QMessageBox.Yes)
            ...
            QMessageBox.critical(windows_main, '护肝宝', "无法连接更新服务器!", QMessageBox.Yes,
                                 QMessageBox.Yes)
    ...
    def change_name(item):
        if item == 0:
            rep = QMessageBox.question(windows_main, '护肝宝', '名称反和谐是利用游戏自带改名\n将动物园改为正常名称\n根据船只多少可能花费数分钟\n是否继续?', QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            if rep == QMessageBox.Yes:
...
def login():
    ...
    try:
        ...
        active_code = config_function.active_code[
            gameData.login_name] if gameData.login_name in config_function.active_code else ""

        need_code = str(gameFunction.get_md5(
            str(datetime.date.year) + str(datetime.date.month) + gameData.login_name + "&^$@(*&^!(&*^!65*//*-")).upper()
        need_code = get_code(need_code)
        while need_code.upper() != active_code.upper():
            active_code, r = QInputDialog.getText(windows_main, "验证", "请输入5位验证码:%s***%s\n"
                                                                   "获取方式:\n"
                                                                   "私聊私聊私聊机器人Z31发送\n"
                                                                   "*验证 %s***%s\n"
                                                                   "后面为你的游戏帐号\n"
                                                                   "软件免费, 仅为防止外泄\n"
                                                                   "唯一群:560271993" % (need_code[0], need_code[-1],
                                                                                      gameData.login_name.lower()[0],
                                                                                      gameData.login_name.lower()[-1]))
            if not r:
                os._exit(0)
            active_code = str(active_code).strip()
            if need_code.upper() != active_code.upper():
                QMessageBox.warning(windows_main, "错误", "验证码错误", QMessageBox.Yes)
        config_function.active_write(username=gameData.login_name, code=active_code)
        th.start()
    except HmError as e:
        message = e.message
